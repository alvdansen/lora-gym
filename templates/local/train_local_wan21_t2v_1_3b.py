"""
Wan 2.1 T2V 1.3B LoRA Training -- Local Edition
=================================================
Trains a character identity LoRA against the Wan 2.1 T2V 1.3B DiT on a
local GPU. This is a lightweight model (~2.84GB) ideal for prototyping
and rapid iteration. Runs on 12GB+ VRAM (RTX 3060, 4060, etc.).

Wan 2.1 differences from Wan 2.2:
  - Single model (no dual high/low noise experts, no --noise_level)
  - bf16 mixed precision (not fp16)
  - No fp8_base (model is small enough without quantization)
  - No fp8_scaled (Wan 2.2 MoE only)
  - No min_timestep / max_timestep / preserve_distribution_shape
  - No Lightning LoRA merge

Usage:
  python train_local_wan21_t2v_1_3b.py
  python train_local_wan21_t2v_1_3b.py --base_dir D:/my_training
  python train_local_wan21_t2v_1_3b.py --lr 5e-5 --dim 8 --alpha 8 --epochs 30
"""
...
MODEL_FILENAMES = {"dit": "wan2.1_t2v_1.3B_bf16.safetensors", "vae": "wan_2.1_vae.safetensors", "t5": "models_t5_umt5-xxl-enc-bf16.pth", "t5_alt": "models_t5_umt5-xxl-enc-bf16.safetensors"}
...
def train(args):
    ...
    if missing:
        ...
        print("Download wan2.1_t2v_1.3B_bf16.safetensors from HuggingFace.")
        sys.exit(1)
    ...
    print("=" * 60)
    print(f"  Wan 2.1 T2V 1.3B LoRA Training (Local)")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Wan 2.1 T2V 1.3B LoRA Training -- Local Edition")
